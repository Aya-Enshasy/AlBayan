...
public class AzkarLoader extends AsyncTask<Void, Void, List<RemembranceDetailsModel>> {

    private static final String TAG = AzkarLoader.class.getSimpleName();
    private static final String JSON_URL = "https://ahegazy.github.io/muslimKit/json/azkar_sabah.json";
    private ProgressBar progressBar;

    private RemembranceDetailsActivity activity;

    public AzkarLoader(RemembranceDetailsActivity activity, ProgressBar progressBar) {
    ...
    protected void onPostExecute(List<RemembranceDetailsModel> quranChapters) {
        ...
        // Save the retrieved data in shared preferences
        String json = convertChaptersListToJson(quranChapters);
        AppPreferences.getInstance(activity).setStringPreference("azkar_sabah", json);

        activity.displayChapters(quranChapters);
